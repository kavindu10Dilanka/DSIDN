...
.catch((error) => {
    ...
    db.collection("approvedByAccFoc").doc(DN.id).delete().then(function()  {
...
.catch((error) => {
    ...
    db.collection("apporovedByAccTbr").doc(DNTBR.id).delete().then(function()  {
    console.log("Document successfully deleted!");
}).catch(function(error)  {
    console.error("Error removing document: ", error);
});
...
deleteDNTBR(DNTBR){
  ...
  db.collection("approvedByAccFoc").onSnapshot(snapshot=>  {
   ...
   db.collection("apporovedByAccTbr").onSnapshot(snapshot=>  {
        snapshot.docChanges().forEach(change=> {
